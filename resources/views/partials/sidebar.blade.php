<aside class="menu-bar menu-sticky">
    <div class="menu-items">
        
        <a href="#" class="link" data-target="applications" data-toggle="tooltip-menu"
            data-tippy-content="Applications"><span class="icon la la-store"></span><span
                class="title">Photon XCS</span></a>
       
        
    
        <a href="{{ route('neutronxcs.index') }}" class="link" data-toggle="tooltip-menu" data-tippy-content="Blank Page"><span
                class="icon la la-file"></span><span class="title">Neutron XCS</span></a>


        <a href="#" target="_blank" class="link" data-toggle="tooltip-menu"
            data-tippy-content="Docs"><span class="icon la la-book-open"></span><span class="title">Charged Particles</span></a>
        
        <a href="#" target="_blank" class="link" data-toggle="tooltip-menu"
            data-tippy-content="Docs"><span class="icon la la-book-open"></span><span class="title">Periodic Table</span></a>

         <a href="#" class="link" data-toggle="tooltip-menu" data-tippy-content="Dashboard"><span
                class="icon la la-laptop"></span><span class="title">About</span></a>
        

    </div>
   
</aside>